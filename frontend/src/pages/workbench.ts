...
async function renderConfigTab(container: HTMLElement, project: api.Project, service: api.McpService) {
  ...
  if (service.id === "yt-mcp") {
    ...
    const exportCmd = "yt-dlp --cookies-from-browser firefox --cookies ~/yt-cookies.txt 2>/dev/null && grep -E '\\.(youtube|google|googlevideo)\\.com' ~/yt-cookies.txt > ~/yt-cookies-filtered.txt && mv ~/yt-cookies-filtered.txt ~/yt-cookies.txt && echo 'Saved to ~/yt-cookies.txt'";
